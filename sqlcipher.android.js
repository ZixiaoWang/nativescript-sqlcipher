...
function Database(dbname, options, callback) {
    ...
    if (dbname !== ""  && dbname !== ":memory:") {
        ...
        var dbFilePath = _getContext().getDatabasePath(dbname);
    ...
    return new Promise(function (resolve, reject) {
        try {
            net.sqlcipher.database.SQLiteDatabase.loadLibs(_getContext());
            if (dbname === ":memory:") {
                //noinspection JSUnresolvedVariable
                self._db = net.sqlcipher.database.SQLiteDatabase.create(null);
            } else {
                //noinspection JSUnresolvedVariable,JSUnresolvedFunction
                if(options && options.key && typeof options.key === 'string' && options.key.length > 0){
                    self._db = net.sqlcipher.database.SQLiteDatabase.openOrCreateDatabase(dbname, options.key, null);
                }else if(typeof options.key !== 'string'){
                    console.warn('Invalid key type, key must be a string.\\nDatabase will not be encrypted.');
